...
typedef struct Tunnel {
	...
	uint8_t blink_ctr;
	uint16_t last_mine_x;
	uint16_t last_mine_y;
	uint16_t wall_thickness_top;
	uint16_t wall_thickness_bottom;
	uint16_t wall_gap;
...
static void Tunnel_advance(Tunnel * const me) {
	...
	if ((rnd < 49U)
			&& ((GAME_TUNNEL_HEIGHT - me->wall_thickness_top
...
static void Tunnel_plantMine(Tunnel * const me) {
	...
	if ((me->last_mine_x + GAME_MINES_DIST_MIN < GAME_TUNNEL_WIDTH)
		...
		if (n < Q_DIM(me->mines)) { /* a disabled Mine found? */
			...
			rnd %= (GAME_TUNNEL_HEIGHT - me->wall_thickness_top
					- me->wall_thickness_bottom - 4U);
			me->last_mine_y = (uint16_t) (me->wall_thickness_top + 2U + rnd);
...
static QState Tunnel_active(Tunnel * const me, QEvt const * const e) {
	...
	case MINE_DISABLED_SIG: {
		Q_ASSERT(
				(Q_EVT_CAST(MineEvt)->id < GAME_MINES_MAX) && (me->mines[Q_EVT_CAST(MineEvt)->id] != (QHsm *)0));
...
static QState Tunnel_show_logo(Tunnel * const me, QEvt const * const e) {
	...
	case Q_ENTRY_SIG: {
		...
		BSP_paintString(16U, (GAME_TUNNEL_HEIGHT / 2U) + 0U,
				"state-machine.com");
...
static QState Tunnel_demo(Tunnel * const me, QEvt const * const e) {
	...
	case Q_ENTRY_SIG: {
		...
		QTimeEvt_armX(&me->blinkTimeEvt, BSP_TICKS_PER_SEC / 2U, BSP_TICKS_PER_SEC / 2U); /* every 1/2 sec */
	...
	case TIME_TICK_SIG: {
		...
		if (me->blink_ctr != 0U) {
			...
			BSP_paintString((GAME_TUNNEL_WIDTH - 10U * 6U) / 2U,
					(GAME_TUNNEL_HEIGHT - 4U) / 2U, "Press BTN0");
		}
		BSP_updateScreen();
		status_ = Q_HANDLED();
		break;
...
static QState Tunnel_playing(Tunnel * const me, QEvt const * const e) {
	...
	case SHIP_IMG_SIG: {
		uint16_t x = Q_EVT_CAST(ObjectImageEvt)->x;
		int16_t y = Q_EVT_CAST(ObjectImageEvt)->y;
	...
	case SCORE_SIG: {
		 ...
		 */
		me->wall_gap = (uint16_t) (GAME_WALLS_GAP_Y
...
static QState Tunnel_game_over(Tunnel * const me, QEvt const * const e) {
	...
	case Q_ENTRY_SIG: {
		...
		BSP_paintString((GAME_TUNNEL_WIDTH - 6U * 9U) / 2U,
				(GAME_TUNNEL_HEIGHT / 2U) - 4U, "Game Over");
		BSP_updateScreen();
		status_ = Q_HANDLED();
		break;
	}
...
static QState Tunnel_screen_saver_show(Tunnel * const me, QEvt const * const e) {
	...
	case Q_ENTRY_SIG: {
		...
		BSP_clearFB();
		BSP_paintString((uint16_t) (rnd % (GAME_TUNNEL_WIDTH - 10U * 6U)),
				(uint16_t) (rnd % (GAME_TUNNEL_HEIGHT - 8U)), "Press BTN0");
		BSP_updateScreen();
